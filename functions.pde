...
// ...or something.
//
// this accumuate is set up for the +/- alternation
...
void accumulate() {
  ...
  if ( frameAvail() ) {
    ...
    if ( add ) {
      ...
      test("subtracting.");
      // cycle through this frame's pixels and subtract them from the totals
      for (int i = 0; i < accNumPixels; i++) {
        // subtract pixel brightness from brightness accumulator
        accumBright[i] -= brightness(inFrame.pixels[i]);
        // add color vector to accumulator
        float angle = hue(inFrame.pixels[i]) * TWO_PI;          // hue angle in radians
        float sat = saturation(inFrame.pixels[i]);
...
/*
// roll off accumulated value if brights and darks are getting too spread out
    if ( brightest < -100 || darkest > 100 ) {
      accumBright[i] *= 0.9;
      colorVectorX[i] *= 0.9;
      colorVectorY[i] *= 0.9;
    }
*/
    ...
    pixSat[i] = dist(0, 0, colorVectorX[i], colorVectorY[i]);
  ...
  test("normalize finished. brightest: " + brightest + " blackest: " + blackest + " most saturated: " + mostSat);
  normalizing = false;
  looking = true;
  viewing = true;
...
void view() {
  if ( viewing ) {
    // set up hue and saturation variables
    float s = 0;
    float h = 0;
...
// this chunk looks at the distribution of light and dark pixels, 
// decides what needs to be done with the BP & WP, and does it
//
void viewMaster() {
  ...
  step = step / 4;
  // look at the value in the middle of the array
  int midPoint = accNumPixels / 2;
  float mid = sortBuffer[ midPoint ] ;

  // evaluate image and figure out what to do with WP & BP
  float adj = ( 0.5 - mid ) * step; 
  // two possibilities
  if ( vmMode ) {
    //mode 1 - bloom whites
    whitePoint -= adj;
