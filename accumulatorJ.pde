 ...
 * and whitepoint.
 *
 * this is the variant that alternates adding a frame and
 * subtracting a frame to accumulate the difference.
 * 
 * 
 * keys:
...
String movieName = "http://127.0.0.1/mov/newHorizonsC.mov";  
...
float threshold = 0.2;        // value to trigger a new accumulation
float[] diffReadings;
...
boolean add = true;           // switch accumulate to add or subtract 
boolean colorOn = true;
boolean writeFile = false;
boolean testMode = true;     // toggles the verbose test mode.
boolean showWork = true;     // toggles whether we see the input image and difference 
...
if ( frameCount % 12 == 0 ) {
 ...
 // viewMaster();    
  ...
  // if we need to write out a file, do it here, before we show our work.
  if ( writeFile ) {
    saveFrame("accumulatorJ-####.jpg");
    test("frame saved.");
    println();
    writeFile = false;
  }
